...
{
	m_acceptorPort = 11112;//by jia
	//GNC::GCS::ConfigurationController::Instance()->readIntGeneral("/DeepWiseCore/PACS/Local", "Puerto", m_acceptorPort, 11112);
	this->m_pNet = NULL;
...
void GIL::DICOM::Service::ThreadFunc()
{
	Start();
	Task();
}
...
CONDITION GIL::DICOM::Service::acceptAssociation()
{
	...
	cond = ASC_receiveAssociation(m_pNet, &assoc, m_rcvPDUSize);//by jia , NULL, NULL, m_TLS, DUL_NOBLOCK, 1);
	...
	}
	{
		...
		if ((cond.bad()) || strcmp(buf, UID_StandardApplicationContext) != 0) {
			...
			std::string cmdName = "Association";
			//{
			...
			GADAPI::PACS::IncomingDicomAssociationCommand* pCmd = new GADAPI::PACS::IncomingDicomAssociationCommand(pCmdParams, cmdName);
			if (pCmd) {
				pCmd->Execute();
			}
			return cond;
